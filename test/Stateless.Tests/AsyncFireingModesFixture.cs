...
public class AsyncFiringModesFixture {
    ...
    public async Task ImmediateModeTransitionsAreInCorrectOrderWithAsyncDriving() {
        ...
        sm.OnTransitioned(t => { record.Add(t.Destination); });

        sm.Configure(State.A)
          .Permit(Trigger.X, State.B);

        sm.Configure(State.B)
          .OnEntryAsync(async () => { await sm.FireAsync(Trigger.Y).ConfigureAwait(false); })
          .Permit(Trigger.Y, State.C);

        sm.Configure(State.C)
          .OnEntryAsync(async () => { await sm.FireAsync(Trigger.Z).ConfigureAwait(false); })
          .Permit(Trigger.Z, State.A);

        await sm.FireAsync(Trigger.X);

        Assert.Equal(new List<State> { State.B, State.C, State.A }, record);
    }
}