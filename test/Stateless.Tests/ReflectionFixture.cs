...
namespace Stateless.Tests
{
    public class ReflectionFixture
    {
        ...
        public void SimpleTransition_Binding()
        {
            ...
            {
                Assert.True(trans.UnderlyingTrigger is Trigger);
                Assert.Equal(Trigger.X, (Trigger)trans.UnderlyingTrigger);
                //
        ...
        public void TwoSimpleTransitions_Binding()
        {
            ...
            foreach (FixedTransitionInfo trans in binding.FixedTransitions)
            {
                ...
                if ((Trigger)trans.UnderlyingTrigger == Trigger.X)
                {
                    Assert.Equal(State.B, (State)trans.DestinationState.UnderlyingState);
                    Assert.False(haveXB);
                    haveXB = true;
                }
                else if ((Trigger)trans.UnderlyingTrigger == Trigger.Y)
                {
        ...
        public void TransitionWithIgnore_Binding()
        {
            ...
            foreach (object ignoredTrigger in binding.IgnoredTriggers)
            {
                Assert.True(ignoredTrigger is Trigger);
                Assert.Equal(Trigger.Y, (Trigger)ignoredTrigger); // Ignored trigger value mismatch
            }
            //
            Assert.Equal(0, binding.Substates.Count());
            Assert.Equal(null, binding.Superstate);
