...
public class StateInfo {
    ...
    internal static void AddRelationships<TState, TTrigger>(StateInfo info,
                                                            StateMachine<TState, TTrigger>.StateRepresentation
                                                                stateRepresentation,
        ...
        foreach (var triggerBehaviours in stateRepresentation.TriggerBehaviours) {
            ...
            foreach (var item in triggerBehaviours.Value.Where(behaviour =>
                                                                   behaviour is StateMachine<TState, TTrigger>
                                                                      .TransitioningTriggerBehaviour)) {
                var destinationInfo =
                    lookupState(((StateMachine<TState, TTrigger>.TransitioningTriggerBehaviour) item).Destination);
            ...
            foreach (var item in triggerBehaviours.Value.Where(behaviour =>
                ...
                var destinationInfo =
                    lookupState(((StateMachine<TState, TTrigger>.ReentryTriggerBehaviour) item).Destination);
                fixedTransitions.Add(FixedTransitionInfo.Create(item, destinationInfo));
            }
